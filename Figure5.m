%% Figure 5
% Simulated data with spurious phase-amplitude coupling using 3 harmonics adding frequency fluctuations. Here I cover 8 different phases every 45 degrees
%
...
%
% signal = (1/j^2)*cos(j*f*2*pi*t + (j-1) * phi;
%
...
%
phi = [2*pi  pi/4  pi/2  3*pi/4  pi  5*pi/4  3*pi/2  7*pi/4];
phistr = {'2*pi'  'pi/4'  'pi/2'  '3*pi/4'  'pi'  '5*pi/4'  '3*pi/2'  '7*pi/4'};
data=[];
for p = 1:size(phi,2);
  ...
  for i = 1:1730;
    ...
    har=[];
    for j=1:3;
      har(j,:) = ((1/j^2)*cos(j*f*2*pi*t + (j-1) * phi(1,p)));
      tmp = tmp + har(j,:);
    end
    s = [s, tmp];
  end
  ...
  title(['base function phi = ' phistr{1,p}]);
  xlabel('Time (s)');
  ...
  %
  data{p}.label{1}='hp';
  data{p}.fsample = fsample;
  data{p}.trial{1} = s;
  data{p}.time{1} = (1:size(data{p}.trial{1},2))./data{p}.fsample;
  data{p}.trialinfo=[1];
  ...
  cfg.length  = 2;
  cfg.overlap = 0.5;
  dataw = ft_redefinetrial(cfg,data{p}); % cut data into epochs
